...
export default function AdminPage() {
  ...
  async function fetchData() {
    const { data: profs, count: mCount } = await supabase.from("profiles").select("*", { count: 'exact' }).order('created_at', { ascending: false });
    const { data: projs, count: pCount } = await supabase.from("projects").select("*", { count: 'exact' }).order('created_at', { ascending: false });
  ...
  async function toggleBlock(userId: string, currentStatus: boolean) {
    ...
    if (!confirm(`Confirm: ${nextStatus ? 'Restrict' : 'Restore'} access for this user?`)) return;
    const { error } = await supabase.from("profiles").update({ is_blocked: nextStatus }).eq("id", userId);
    if (!error) fetchData();
  }

  async function removeUser(userId: string) {
    if (!confirm("⚠️ PERMANENT TERMINATION: Remove this member?")) return;
  ...
  async function wipeMessage(id: string) {
    if (!confirm("Wipe message from transmission log?")) return;
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-12 border-b border-red-500/20 pb-10 flex flex-col md:flex-row justify-between items-end gap-6">
          <div>
            ...
            <p className="font-space-mono text-[10px] text-zinc-500 tracking-[0.3em] mt-2 uppercase">Root Level Authorization Verified</p>
          </div>
          <div className="flex gap-10">
            ...
            <div className="text-center border-l border-zinc-800 pl-10">
              <p className="text-[9px] text-zinc-600 uppercase font-space-mono mb-1 tracking-widest">Global Projects</p>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
              ...
              {members.map(m => (
                <div key={m.id} className={`p-5 rounded-3xl border transition-all ${m.is_blocked ? 'border-red-500/30 bg-red-500/5' : 'border-zinc-800 bg-zinc-900/40'}`}>
                  ...
                  <div className="flex gap-2 mt-4">
                    {m.is_admin ? (
                      <span className="text-[8px] bg-red-600/20 text-red-500 px-3 py-1 rounded-full font-black uppercase tracking-tighter">Root Admin</span>
                    ) : (
                      <>
                        <button onClick={() => toggleBlock(m.id, m.is_blocked)} className={`px-4 py-1.5 rounded-full text-[8px] font-bold uppercase ${m.is_blocked ? 'bg-emerald-600 text-black' : 'bg-zinc-800 text-zinc-400 hover:text-red-500'}`}>
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-3">
              {projects.length === 0 ? <p className="text-[10px] text-zinc-700 italic">No project data found.</p> : projects.map(p => (
                <div key={p.id} className="p-5 bg-zinc-900/40 border border-zinc-800 rounded-3xl group hover:border-red-500/20 transition-all">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="text-[11px] font-michroma text-zinc-200 uppercase leading-tight">{p.title}</h3>
                  ...
                  <p className="text-[9px] text-zinc-600 font-space-mono uppercase tracking-widest italic">Author: {p.author_name || 'Anonymous'}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-4">
              {messages.length === 0 ? <p className="text-[10px] text-zinc-700 italic text-center py-10">Inbox Empty.</p> : messages.map(msg => (
                <div key={msg.id} className="bg-zinc-900/60 border border-zinc-800 p-6 rounded-3xl group hover:border-emerald-500/20 transition-all">
                  ...
                  <div className="bg-black/40 p-4 rounded-2xl border border-zinc-800/50">
                    <p className="text-xs text-zinc-400 leading-relaxed font-outfit">{msg.content}</p>
                  </div>
                </div>
              ))}
            </div>
